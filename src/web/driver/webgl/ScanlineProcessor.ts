...
class ScanlineProcessor implements Processor {
    ...
    init(): void {
        ...
        this._program = compileProgram(gl, vsh.plain.source, fsh.scanlines.source);
    ...
    render(texture: WebGLTexture): void {
        ...
        gl.uniform1f(getUniformLocation(gl, this._program.program, fsh.scanlines.uniform.level), 1 - this._level);
        gl.uniform1f(getUniformLocation(gl, this._program.program, fsh.scanlines.uniform.height), 2 * this._height);
        gl.uniform1i(getUniformLocation(gl, this._program.program, fsh.scanlines.uniform.textureUnit), 0);
